...
void sort_chunks() {
    ...
    for (i = 0; i < NUMBER_OF_CHUNKS; i++)
    {
        ...
        for (j = i+1; j < NUMBER_OF_CHUNKS; j++)
            if (chunks[j][0] < chunks[min_chunks][0])
                min_chunks = j;


        if(min_chunks != i)
            swap_strings(&chunks[min_chunks][0], &chunks[i][0]);
    }
...
void decrypt_chunks()
{
    ...
    {
        for(int b = 1 ; b < CHUNK_LENGTH -1 ; b++)
        {
            yptr = chunks[a][b];
            yptr = yptr - DECRYPTION_SHIFT;
            chunks[a][b] = yptr;
...
void display_chunks()
{
    ...
    {
        for (y = 0; y < CHUNK_LENGTH - 1 ; y++)
        {
            printf("%c", xptr[y+1]);
        }
        printf("\n");
        xptr = xptr + CHUNK_LENGTH;
    }
}